#!/usr/bin/env bash
# Execute one blinded, fresh-worktree Hermes task arm for EXP-007.
set -euo pipefail

ROOT=/Users/<user>/HermesProjects/Local-LLM-Lab
BENCH="$ROOT/results/raw/EXP-007/real-repo-ab"
ASSIGNMENT="$BENCH/private/assignment.json"
PROFILE="$HOME/.local/bin/exp007bench"
...
FIXTURE="$BENCH/fixtures/T01-ios-layout"
PROMPT="$BENCH/prompts/T01/task.md"
ORACLE="$BENCH/oracles/T01/check.py"
...
# Preserve candidate-independent controls before launch. This output intentionally never contains model identity.
{
  ...
  shasum -a 256 "$PROMPT"
  shasum -a 256 "$ROOT/results/raw/EXP-007/context-safe-harness/POLICY.md"
...
HERMES_HOME="$HOME/.hermes/profiles/exp007bench" "$HERMES_PY" - >"$RUN/context-safe-hook-check.txt" 2>&1 <<'PY'
from hermes_cli import plugins
from hermes_cli.lifecycle import has_hook
plugins.discover_plugins()
if not has_hook("transform_tool_result"):
    raise SystemExit("EXP-007 context-safe hook is not loaded")
print("EXP-007 context-safe hook loaded")
PY
...
PY
python3 "$ROOT/tools/exp007_summarize_context_metrics.py" "$RUN" \
