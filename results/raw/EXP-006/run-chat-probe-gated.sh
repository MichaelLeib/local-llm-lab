#!/usr/bin/env bash
# Approval-gated chat-template probe. Do not remove the gate.
set -euo pipefail
...
[[ -x "$CLI" && -f "$MODEL/manifest.json" && -f "$MODEL/verified-install.json" && -f "$MESSAGES" ]] || {
  echo "Refusing inference: verified inputs are missing." >&2
  exit 2
}
