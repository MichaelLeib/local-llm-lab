#!/usr/bin/env bash
# EXP-006 optimization sprint: clean 16-slot trace comparator.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
OUT="$(cd "$(dirname "$0")" && pwd)/baseline-16slot-clean-trace"
...
{
  date -u
  printf 'arm=baseline-16slot-clean-trace\n'
  sysctl vm.swapusage
  memory_pressure -Q
  df -h /
  TINYTITAN_DECODE_IO_TRACE=1 "$CLI" --model "$MODEL" --messages-file "$MESSAGES" --max-new 64 --max-context 4096 --temperature 0 --seed 12345 --thinking off --concise --expert-cache-slots 16
  status=$?
  printf 'probe_exit=%s\n' "$status"
  sysctl vm.swapusage
  memory_pressure -Q
