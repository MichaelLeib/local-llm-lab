#!/usr/bin/env bash
# Execute one fresh-label sealed-pair Hermes task arm under frozen EXP-007 context-safe-v2.
set -euo pipefail

ROOT=/Users/<user>/HermesProjects/Local-LLM-Lab
SOURCE_BENCH="$ROOT/results/raw/EXP-007/real-repo-ab"
BENCH="$ROOT/results/raw/EXP-007/real-repo-ab-v2-scoreable"
...
# Preserve candidate-independent controls before launch. This output intentionally never contains model identity.
{
  ...
  printf 'blind_candidate=%s\nharness=hermes\ntask=%s\nassignment_mode=fresh-v2-sealed-pair\n' "$CID" "$TASK"
  shasum -a 256 "$ASSIGNMENT"
  shasum -a 256 "$PROMPT"
  shasum -a 256 "$POLICY"
