...
class ContextSafeOutputTests(unittest.TestCase):
    ...
    def test_search_caps_match_records_and_reports_omission(self):
        ...
        self.assertLessEqual(len(payload["matches"]), self.module.MAX_SEARCH_MATCHES)
        self.assertEqual(payload["exp007_context_safe"]["omitted_matches"], 80 - len(payload["matches"]))
        self.assertIn(f"returned first {len(payload['matches'])} of 80 records", payload["matches_truncation"])
        self.assertIn("narrow pattern/path", payload["matches_truncation"])
        self.assertLessEqual(token_count(self.module, transformed), self.module.MAX_RESULT_TOKENS)
