...
outdir = base / "prefill-sweep-clean"
...
for n in arms:
    ...
    run = {"arm": n, "started_at": now(), "settings": {"slots": 16, "max_context": 4096, "max_new": 16, "temperature": 0, "seed": 12345, "prefill_chunk": "auto"}}
    run["swap_before"] = capture(["/usr/sbin/sysctl", "vm.swapusage"])[1].strip()
    run["memory_before"] = capture(["/usr/bin/memory_pressure", "-Q"])[1].strip()
    t0 = time.monotonic()
    rc, out, err = capture([str(src / ".build/release/slipstream"), "--model", str(model), "--prompt", prompt, "--max-new", "16", "--max-context", "4096", "--temperature", "0", "--seed", "12345", "--expert-cache-slots", "16", "--prefill-chunk", "auto"])
