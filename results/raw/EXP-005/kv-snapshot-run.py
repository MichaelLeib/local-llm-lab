...
for name in ["cold-save", "warm-restore"]:
    ...
    row={"name":name,"started_at":dt.datetime.now(dt.timezone.utc).isoformat(),"command":cmd,"exit_code":p.returncode,"wall_seconds":round(wall,3),"stdout":p.stdout,"stderr":p.stderr,"swap_before":before,"swap_after":after}
    rows.append(row)
