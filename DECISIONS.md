...
## D-030 — Stop resident Nemotron 3.5 Lightning Branch 1 at first-load pressure (2026-09-21)

Do not advance the verified 9.09-GiB `NVIDIA-Nemotron-3.5-Lightning-30B-A3B` mixed Q2/Q4 2.47-bpw GGUF to populated 8K, 46K/64K/128K, API/tool, Hermes, or a higher-bpw download on this 16 GiB M3. Its isolated current llama.cpp Metal load with one sequence and Q8 K/V reached 13% free system memory against a recorded 63% quiet baseline before a populated request, so the independent guard terminated it at the 15% floor. This is a resident-weight/Metal unified-memory failure before KV is material; lower KV precision, batch tuning, or prefix caching cannot reclaim the missing multi-GiB headroom. Do not evade the result with a destructive sub-2.47-bpw representation: the raw smoke already requires a real template/quality check, and no evidence supports that extreme additional compression as a useful agent. Revisit only with a demonstrably bounded-working-set Metal runtime, 24–32 GiB+ unified memory, or an explicitly separate and approved SSD/expert-streaming branch.
